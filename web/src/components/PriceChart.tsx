...
export function PriceChart({ bars, indicators, overlays, signals = [] }: PriceChartProps) {
  ...
  useEffect(() => {
    ...
    const chartTime = (value: string): Time => value.length > 10 ? Math.floor(new Date(value).getTime() / 1000) as Time : value as Time
    candles.setData(bars.map((bar) => ({ time: chartTime(bar.date), open: bar.open, high: bar.high, low: bar.low, close: bar.close })))
    ...
    const addLine = (field: keyof IndicatorPoint, color: string, dashed = false) => {
      ...
      series.setData(bars.flatMap((bar) => {
        ...
        return typeof value === 'number' ? [{ time: chartTime(bar.date), value }] : []
      }))
    }
    ...
    volume.setData(bars.map((bar) => ({ time: chartTime(bar.date), value: bar.volume, color: bar.close >= bar.open ? '#c2413a4d' : '#138a634d' })))
